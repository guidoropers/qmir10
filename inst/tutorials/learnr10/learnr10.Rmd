---
title: "Week 10 - Uncertainty and Inference"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: false
...
question("What is the standard error?",
  answer("It is the standard deviation of an estimate that is captured over many repeated samples.", correct = T),
...
Let's look again at the model that you already had to specify in last week's exercise. As you might remember, you had to run a regression with `conflict_p` as dependent variable and `seatshare` as the main independent variable of interest. The working hypothesis was that larger opposition parties oppose the government more often than smaller ones. 

To control for potential confounders, the model also included the following following control variables:
